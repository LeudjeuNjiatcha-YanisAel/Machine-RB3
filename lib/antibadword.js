...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    ...
    if (match === 'on') {
        ...
        return sock.sendMessage(chatId, { text: '*AntiBadword a ete activer. Utilisation *antibadword set <action> to customize action*' }, { quoted: message });
    }
    ...
    if (match === 'off') {
        ...
        return sock.sendMessage(chatId, { text: '*AntiBadword a ete desactiver pour ce groupe*' }, { quoted: message } );
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '*Action invalid. Choisi: delete, kick, or warn*' }, { quoted: message } );
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `*AntiBadword action set to: ${action}*` }, { quoted: message } );
    }

    return sock.sendMessage(chatId, { text: '*Command invalide. Utilisation *antibadword pour voir l\'usage*' }, { quoted: message } );
}
